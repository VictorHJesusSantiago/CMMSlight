-- Ordens de servico: assinatura digital simples e vinculo de reabertura (retrabalho)
...
-- Timeline/comentarios/eventos da OS
CREATE TABLE work_order_event (
    id              BIGSERIAL PRIMARY KEY,
    work_order_id   BIGINT NOT NULL REFERENCES work_order(id) ON DELETE CASCADE,
    event_type      VARCHAR(30) NOT NULL, -- STATUS_CHANGE, COMMENT, ASSIGNMENT, SIGNATURE, CHECKLIST, PART_USED
...
-- Anexos/evidencias fotograficas da OS (antes/depois)
CREATE TABLE work_order_attachment (
    ...
    category        VARCHAR(20) NOT NULL DEFAULT 'OTHER', -- BEFORE, AFTER, OTHER
    uploaded_at     TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX idx_wo_attachment_wo ON work_order_attachment(work_order_id);

-- Falhas: classificacao e analise de causa raiz estruturada (5 porques)
ALTER TABLE failure_history ADD COLUMN classification VARCHAR(30) NOT NULL DEFAULT 'OTHER'; -- MECHANICAL, ELECTRICAL, OPERATIONAL, OTHER
...
-- Checklist tipado (sim/nao, texto, numero, multipla escolha)
ALTER TABLE checklist_item ADD COLUMN item_type VARCHAR(20) NOT NULL DEFAULT 'YES_NO'; -- YES_NO, TEXT, NUMBER, MULTIPLE_CHOICE
ALTER TABLE checklist_item ADD COLUMN options JSONB; -- lista de opcoes para MULTIPLE_CHOICE
